...
class User2 extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            [['currentPassword','newPassword','newPasswordConfirm'], 'required', 'message'=>'Не может быть пустым'],
    ...
    public function validateCurrentPassword(){
        if(!$this->verifyPassword($this->currentPassword)){
            $this->addError('currentPassword','Не правельный пароль');
        }
    }
    public function verifyPassword($password){
        $dbp=static::findOne(['username'=>Yii::$app->user->identity->username, 'status'=>self::STATUS_ACTIVE])->password_hash;
        return Yii::$app->security->validatePassword($password, $dbp);
    }
    /**
     * @inheritdoc
     */
